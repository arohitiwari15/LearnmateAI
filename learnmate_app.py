...
GEMINI_VIS_URL = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={GEMINI_API_KEY}"
...
def call_gemini(prompt: str, system: str = "", max_tokens: int = 800) -> str:
    ...
    try:
        ...
        if "candidates" not in data:
            return f"⚠️ API Error: {json.dumps(data)}"
...
def get_explanation(level: str, topic: str, subject: str, context: str = "") -> str:
    ...
    return call_gemini(prompt, system=TUTOR_SYSTEM)
...
def chat_with_tutor(history: list, user_msg: str, level: str, subject: str, notes: str = "") -> str:
    ...
    return call_gemini(prompt, system=TUTOR_SYSTEM + context, max_tokens=700)


def gemini_visualize(topic: str, subject: str) -> str:
    prompt = f"""Create a vivid visual explanation / mental diagram of '{topic}' in {subject}.
Describe it like drawing on a whiteboard for a student. Include:
🔷 Core Concept (one clear sentence)
📊 Diagram Description (describe with arrows, labels, boxes)
🔗 How the parts connect
💡 The 'aha moment' — one memorable insight
🎯 Common mistakes students make

Use emojis, arrows (→, ↓, ⟷), and clear formatting."""
    return call_gemini(prompt, max_tokens=600)


def summarize_notes(text: str, subject: str) -> str:
    prompt = f"Summarize the key concepts from these student notes about {subject}. List main topics and important points:\n\n{text[:2000]}"
...
.card {{ background: {BG_CARD}; border: 1px solid {BR_CARD}; border-radius: 14px; padding: 1.3rem 1.5rem; margin-bottom: 0.9rem; transition: border-color 0.2s, transform 0.15s; }}
.card:hover {{ border-color: #5b21b6; transform: translateY(-1px); }}
.card-purple {{ background: {"linear-gradient(135deg,#1c0f38 0%,#13101e 100%)" if D else "linear-gradient(135deg,#ede9fe,#f5f3ff)"}; border: 1px solid {"#4c1d95" if D else "#c4b5fd"}; border-radius: 14px; padding: 1.3rem 1.5rem; margin-bottom: 0.9rem; }}
.hero {{ background: {BG_HERO}; border: 1px solid {BR_HERO}; border-radius: 18px; padding: 2.2rem; margin-bottom: 1.5rem; position: relative; overflow: hidden; }}
.hero::after {{ content: ''; position: absolute; top: -60%; right: -10%; width: 380px; height: 380px; background: radial-gradient(circle, rgba(139,92,246,0.1) 0%, transparent 68%); pointer-events: none; }}
...
.stButton > button {{ background: linear-gradient(135deg, {AC_BTN1}, {AC_BTN2}) !important; color: #ede8ff !important; border: none !important; border-radius: 10px !important; padding: 0.6rem 1.4rem !important; font-family: 'Sora', sans-serif !important; font-weight: 600 !important; font-size: 0.86rem !important; transition: all 0.2s !important; }}
.stButton > button:hover {{ background: linear-gradient(135deg, {AC_HOVER}, {AC_BTN1}) !important; transform: translateY(-1px) !important; box-shadow: 0 6px 22px rgba(91,33,182,0.35) !important; }}
.stRadio > div[role="radiogroup"] {{ gap: 0.3rem !important; display: flex !important; flex-direction: column !important; }}
.stRadio > div[role="radiogroup"] > label {{ display: flex !important; align-items: center !important; background: {BG_INPUT} !important; border: 1px solid {BR_INPUT} !important; border-radius: 9px !important; padding: 0.55rem 1rem !important; font-size: 0.88rem !important; color: {TX_MAIN} !important; cursor: pointer !important; transition: all 0.15s !important; width: 100% !important; margin: 0 !important; }}
.stRadio > div[role="radiogroup"] > label:hover {{ border-color: #6d28d9 !important; color: {"#d4c8ff" if D else "#4c1d95"} !important; background: {"#1a1030" if D else "#f5f3ff"} !important; }}
.stRadio label span, .stRadio label p {{ color: {TX_MAIN} !important; display: block !important; }}
.bubble-user {{ background: {BG_BUSER}; border: 1px solid {"#4c1d95" if D else "#c4b5fd"}; border-radius: 14px 14px 4px 14px; padding: 0.85rem 1.1rem; margin: 0.5rem 0 0.5rem 18%; font-size: 0.88rem; color: {TX_BUSER}; line-height: 1.65; }}
.bubble-ai {{ background: {BG_BAI}; border: 1px solid {"#1c1c34" if D else "#e2deff"}; border-radius: 14px 14px 14px 4px; padding: 0.85rem 1.1rem; margin: 0.5rem 18% 0.5rem 0; font-size: 0.88rem; color: {TX_BAI}; line-height: 1.75; }}
.sender {{ font-size: 0.65rem; color: {TX_DIM}; letter-spacing: 0.08em; text-transform: uppercase; margin-bottom: 0.3rem; }}
.ai-box {{ background: {BG_AI}; border: 1px solid {BR_AI}; border-left: 3px solid #7c3aed; border-radius: 0 12px 12px 0; padding: 1.3rem 1.5rem; margin-top: 0.8rem; font-size: 0.9rem; line-height: 1.8; color: {TX_AI}; white-space: pre-wrap; }}
.vis-box {{ background: {BG_VIS}; border: 1px solid {BR_VIS}; border-left: 3px solid #06b6d4; border-radius: 0 12px 12px 0; padding: 1.3rem 1.5rem; margin-top: 0.8rem; font-size: 0.88rem; line-height: 1.85; color: {TX_VIS}; white-space: pre-wrap; }}
[data-testid="metric-container"] {{ background: {BG_CARD2} !important; border: 1px solid {BR_CARD} !important; border-radius: 12px !important; padding: 1rem 1.2rem !important; }}
...
.stTextInput > div > div > input, .stTextArea textarea, .stChatInput textarea {{ background: {BG_INPUT} !important; border: 1px solid {BR_INPUT} !important; border-radius: 10px !important; color: {TX_MAIN} !important; font-family: 'Sora', sans-serif !important; font-size: 0.88rem !important; }}
.stTextInput > div > div > input:focus, .stTextArea textarea:focus {{ border-color: #6d28d9 !important; box-shadow: 0 0 0 3px rgba(109,40,217,0.18) !important; }}
.stSelectbox > div > div {{ background: {BG_INPUT} !important; border: 1px solid {BR_INPUT} !important; border-radius: 10px !important; color: {TX_MAIN} !important; }}
...
with st.sidebar:
    ...
    for icon, label, pg in [("🏠","Home","home"),("📝","Quiz","quiz"),("💬","Chat Tutor","chat"),("📄","My Notes","notes")]:
    ...
    if st.session_state.score_history:
        ...
        st.markdown(f'<div style="font-size:0.82rem;color:#7a7498;line-height:2.2">🏆 Best: <strong style="color:#d4d0f0">{best}/5</strong><br>📊 Avg: <strong style="color:#d4d0f0">{avg:.1f}/5</strong><br>🔄 Quizzes: <strong style="color:#d4d0f0">{st.session_state.attempt_count}</strong></div>', unsafe_allow_html=True)
...
if st.session_state.page == "home":
    ...
    st.markdown(f'<div class="hero"><h1>{greeting}<br><span class="accent">Ready to learn something?</span></h1><p>Your personal AI tutor — diagnoses your level, explains concepts in your language, and adapts to you.</p></div>', unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3)
    for col, icon, title, desc, btn_label, btn_key, target in [
        (c1,"📝","Smart Quiz","AI writes 5 questions for your subject. We find your gaps then fix them.","Take a Quiz →","h_quiz","quiz"),
        (c2,"💬","Chat Tutor","Ask anything. Get clear answers with analogies and step-by-step breakdowns.","Open Chat →","h_chat","chat"),
        (c3,"📄","Upload Notes","Paste your notes. The tutor references them for personalised help.","Upload Notes →","h_notes","notes"),
    ]:
        with col:
            st.markdown(f'<div class="card"><div style="font-size:1.7rem;margin-bottom:0.5rem">{icon}</div><div style="font-weight:600;color:#d8d4f0;margin-bottom:0.4rem">{title}</div><div style="font-size:0.8rem;color:#5a5870;line-height:1.65">{desc}</div></div>', unsafe_allow_html=True)
    ...
    if st.session_state.score_history:
        ...
        if len(st.session_state.score_history) >= 2:
            st.line_chart({"Attempt":list(range(1,len(st.session_state.score_history)+1)),"Score":st.session_state.score_history}, x="Attempt", y="Score", height=180, use_container_width=True)

    st.markdown("---")
    st.markdown('<div class="sec">Quick Explain</div>', unsafe_allow_html=True)
    qc1, qc2 = st.columns([5, 1])
    with qc1:
        qt = st.text_input("Quick explain topic", placeholder=f"Type any topic in {st.session_state.subject}…  e.g. recursion, photosynthesis", label_visibility="collapsed", key="quick_t")
    with qc2:
        if st.button("Explain 🔮", key="quick_btn", use_container_width=True) and qt:
            with st.spinner("Thinking…"):
                st.session_state.current_explanation = get_explanation(st.session_state.level or "Beginner", qt, st.session_state.subject, st.session_state.notes_context)
    ...
    if st.session_state.current_explanation:
        ...
        if st.button("🎨  Visualize It", key="h_vis"):
...
# ─────────────────────────────────────────────────────────────
    ...
    st.markdown(f'<div class="hero" style="padding:1.7rem 2rem"><h1 style="font-size:1.6rem">📝 Diagnostic Quiz</h1><p>Subject: <span class="accent">{st.session_state.subject}</span> — AI generates 5 fresh questions every time</p></div>', unsafe_allow_html=True)
    ...
    if not st.session_state.quiz_submitted:
        if not st.session_state.quiz_questions:
            ...
            for i, q in enumerate(questions):
                opts = q.get("options", {})
                st.markdown(f'<div class="q-wrap"><div class="q-num">Q{i+1} · {q.get("topic","")}</div><div class="q-text">{q["question"]}</div></div>', unsafe_allow_html=True)
                choice = st.radio(label=f"Question {i+1} options", options=list(opts.keys()),
            ...
            with rc1:
                if st.button("✅  Submit", key="submit_q", use_container_width=True):
                    if any(st.session_state.quiz_answers.get(i) is None for i in range(5)):
                        ...
                        score = 0; weak = []
                        for i, q in enumerate(questions):
                            if st.session_state.quiz_answers.get(i) == q["answer"]: score += 1
                            else: weak.append(q.get("topic", f"Topic {i+1}"))
                        ...
                        st.rerun()
            with rc2:
                if st.button("🔁  New questions", key="regen_q"):
        ...
        st.markdown(f"""<div class="hero"><div style="display:flex;align-items:center;gap:2.5rem;flex-wrap:wrap">
        <div><div class="sec">Score</div><div class="score-num" style="color:{col}">{score}<span style="font-size:1.4rem;color:#3a3858">/5</span></div><div style="font-size:0.78rem;color:#5a5870;margin-top:0.2rem">{score*20}% correct</div></div>
        <div><div class="sec">Level Detected</div><span class="badge {bmap.get(level)}">{imap.get(level)} {level}</span>
        <div style="margin-top:0.7rem;font-size:0.82rem;color:#7a7498">{"🎉 Outstanding!" if score>=4 else "💪 Solid work!" if score==3 else "🔧 Let's fix the gaps."}</div></div>
        ...
        for i, q in enumerate(questions):
            ...
            correct_text = html_lib.escape(opts.get(q["answer"], q["answer"]))
            user_text    = html_lib.escape(opts.get(ua, ua or "Not answered"))
            ...
            <div style="font-size:0.82rem;color:{tc}">{icon} Your answer: {user_text}</div>
            {"" if correct else f'<div style="font-size:0.82rem;color:#6ee7b7;margin-top:0.15rem">✔ Correct: {correct_text}</div>'}
            ...
            with ec1:
                if st.button("📖  Explain This Topic", key="r_explain"):
                    with st.spinner(f"Explaining {sel}…"):
                        ...
                        st.session_state.visual_explanation = ""
            with ec2:
                if st.button("🎨  Visualize It", key="r_vis"):
                    with st.spinner("Building mental diagram…"):
...
# ─────────────────────────────────────────────────────────────
    ...
    st.markdown(f'<div class="hero" style="padding:1.6rem 2rem"><h1 style="font-size:1.5rem">💬 Chat with your Tutor</h1><p>Ask anything about <span class="accent">{st.session_state.subject}</span>. Level: <span class="accent">{st.session_state.level or "not set yet"}</span></p></div>', unsafe_allow_html=True)

    if st.session_state.notes_context:
        st.markdown('<div style="font-size:0.76rem;color:#6ee7b7;margin-bottom:0.8rem">📄 Your notes are active.</div>', unsafe_allow_html=True)

    if not st.session_state.chat_history:
        nm  = st.session_state.student_name or "there"
        lvl = st.session_state.level or "Beginner"
        st.markdown(f'<div class="bubble-ai"><div class="sender">🌱 LearnMate</div>Hey {nm}! I\'m your AI tutor for <strong>{st.session_state.subject}</strong>.<br>I\'ll match your <strong>{lvl}</strong> level. What would you like to understand today? 🙂</div>', unsafe_allow_html=True)
    ...
    for msg in st.session_state.chat_history:
        if msg["role"] == "user":
            ...
            st.markdown(f'<div class="bubble-ai"><div class="sender">🌱 LearnMate</div>{msg["content"].replace(chr(10),"<br>")}</div>', unsafe_allow_html=True)

    st.markdown("---")
    suggs = {
        "Data Structures & Algorithms": ["Explain recursion","What's Big O?","How does a hash table work?"],
        "Mathematics":  ["Explain derivatives","What is integration?","How to solve quadratics?"],
        "Physics":      ["Explain Newton's laws","What is entropy?","How does electricity work?"],
        ...
        "Literature":   ["Theme vs plot?","Explain narrative structure","What is a metaphor?"],
        "Economics":    ["Explain supply and demand","What causes inflation?","What is GDP?"],
    }
    chips = suggs.get(st.session_state.subject, ["Explain the basics","Give me an example","Quiz me"])
    ...
    for ci, (chip, ccol) in enumerate(zip(chips, ccols)):
        with ccol:
            if st.button(chip, key=f"chip_{ci}", use_container_width=True):
                with st.spinner("Thinking…"):
                    reply = chat_with_tutor(st.session_state.chat_history, chip, st.session_state.level or "Beginner", st.session_state.subject, st.session_state.notes_context)
    ...
    if user_msg:
        with st.spinner("🤔 Thinking…"):
            reply = chat_with_tutor(st.session_state.chat_history, user_msg, st.session_state.level or "Beginner", st.session_state.subject, st.session_state.notes_context)
...
# ─────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="hero" style="padding:1.6rem 2rem"><h1 style="font-size:1.5rem">📄 Your Notes</h1><p>Paste text or upload a .txt file. The tutor uses your notes to personalise every explanation and quiz.</p></div>', unsafe_allow_html=True)

    t1, t2 = st.tabs(["📋 Paste Text", "📁 Upload .txt File"])
    with t1:
        st.markdown('<div class="sec">Paste your notes</div>', unsafe_allow_html=True)
        pasted = st.text_area("Paste notes", placeholder="Paste your class notes, textbook text, any study material…", height=240, label_visibility="collapsed", key="paste_area")
    ...
    with t2:
        ...
        if uploaded:
            ...
            st.markdown(f'<div style="font-size:0.78rem;color:#4a4862;margin-bottom:0.5rem">📄 {uploaded.name} — {len(content):,} chars</div>', unsafe_allow_html=True)
    ...
    if st.session_state.notes_context:
        st.markdown("---")
        st.markdown(f'<div class="card-purple"><div style="font-size:0.76rem;color:#a78bfa;margin-bottom:0.4rem">✅ Notes Active</div><div style="font-size:0.82rem;color:#7a7498;line-height:1.6">{st.session_state.notes_summary[:280]}…</div></div>', unsafe_allow_html=True)
        nc1, nc2, nc3 = st.columns(3)
        with nc1:
            if st.button("🗑  Clear Notes", key="clr_notes"):
                st.session_state.notes_context = ""; st.session_state.notes_summary = ""; st.rerun()
        with nc2:
            if st.button("📝  Quiz Me on Notes", key="n_quiz"):
                st.session_state.quiz_questions = []; st.session_state.page = "quiz"; st.rerun()
        with nc3:
            if st.button("💬  Chat About Notes", key="n_chat"):
                st.session_state.page = "chat"; st.rerun()

        st.markdown("---")
        nq1, nq2 = st.columns([5, 1])
        with nq1:
            nq = st.text_input("Ask about notes", placeholder="e.g. What are the key points? What should I memorise?", label_visibility="collapsed", key="n_ask")
        with nq2:
            if st.button("Ask →", key="n_ask_btn", use_container_width=True) and nq:
                with st.spinner("Reading your notes…"):
                    reply = chat_with_tutor([], nq, st.session_state.level or "Beginner", st.session_state.subject, st.session_state.notes_context)

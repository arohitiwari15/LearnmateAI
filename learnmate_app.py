...
# ─────────────────────────────────────────────────────────────
# KEYS — loaded from Streamlit secrets (Settings → Secrets)
# Add these in Streamlit Cloud: OPENROUTER_API_KEY and GEMINI_API_KEY
# For local dev: create .streamlit/secrets.toml with the same keys
# ─────────────────────────────────────────────────────────────
...
def call_llama(messages: list, max_tokens: int = 800) -> str:
    if not OPENROUTER_API_KEY:
        return "⚠️ OPENROUTER_API_KEY not set. Add it in Streamlit secrets."
    headers = {
...
def gemini_visualize(topic: str, subject: str) -> str:
    if not GEMINI_API_KEY:
        return "⚠️ GEMINI_API_KEY not set. Add it in Streamlit secrets."
